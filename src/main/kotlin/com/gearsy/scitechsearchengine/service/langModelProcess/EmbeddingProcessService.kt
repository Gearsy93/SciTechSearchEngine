...
import com.gearsy.scitechsearchengine.model.pythonEmbedding.EmbeddingResponse
import com.gearsy.scitechsearchengine.model.pythonEmbedding.RubricEmbeddingRequest
...
class EmbeddingProcessService(private val modelLoader: ModelLoaderService,
    ...
    suspend fun requestPythonEmbedding(term: String, context: List<String>, title: String): List<Float> {
        ...
        val response: EmbeddingResponse = client.post("http://localhost:8001/embedding") {
            contentType(ContentType.Application.Json)
            setBody(EmbeddingRequest(term, context, title))
        }.body()
    ...
    suspend fun requestBatchEmbeddings(requests: List<EmbeddingRequest>): List<List<Float>> {
        ...
        val response: List<EmbeddingResponse> = client.post("http://localhost:8001/embedding/batch") {
            contentType(ContentType.Application.Json)
            setBody(requests)
        }.body()
    ...
    suspend fun requestRubricEmbedding(title: String, terms: List<String>): List<Float> {
        ...
        val payload = RubricEmbeddingRequest(title, terms)

        val response: EmbeddingResponse = client.post("http://localhost:8001/embedding/rubric") {
            contentType(ContentType.Application.Json)
            setBody(payload)
        }.body()
